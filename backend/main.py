...
def validate_tickers_endpoint(tickers: List[str] = Query(...)):
    result = _validate_tickers(tickers)
    return result
...
def optimize(req: PortfolioRequest):
    ...
    # 2. Classical Markowitz optimization
    try:
    ...
    # Convert QAOA binary vector to weights (equal weight among selected stocks)
    selected = qaoa_binary.astype(bool)
    qaoa_weights = np.zeros(n)
    if selected.any():
        qaoa_weights[selected] = 1.0 / selected.sum()
    else:
        qaoa_weights = np.ones(n) / n  # fallback equal weight

    # 5. Compute metrics
    qaoa_metrics = compute_portfolio_metrics(
        qaoa_weights, stock_data.mean_returns, stock_data.cov_matrix
    )
    ...
    # 7. Build allocations as percentage dicts
